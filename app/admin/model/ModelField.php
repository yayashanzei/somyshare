<?php
	...
	class ModelField extends \think\Model {
		...
		public function update( $data = null , $create = true ) {
			/* 获取数据对象 */
			$data = empty( $data ) ? $this->create() : $data;
		...
		protected function checkTableExist( $model_id ) {
			...
			$table_name = $this->tableName = C( 'database.prefix' ) . strtolower( $model['name'] );
...
sql;
		...
		protected function addField( $field ) {
			...
			if ( $field['field_default'] === '' ) {
				$default = '';
			} elseif ( is_numeric( $field['field_default'] ) ) {
				$default = ' DEFAULT ' . $field['field_default'];
			} elseif ( is_string( $field['field_default'] ) ) {
				$default = ' DEFAULT \'' . $field['field_default'] . '\'';
			} else {
				$default = '';
			}
...
ADD COLUMN `{$field['name']}`  {$field['field_define']} {$default} COMMENT '{$field['remark']}';
sql;
			} else {
				//新建表时是否默认新增“id主键”字段
				$model_info                = M( 'Model' )->field( 'engine_type,need_pk' )->getById( $field['model_id'] );
				$model_info['engine_type'] = S( 'enum' )[80]['_'][108]['_'][ $model_info['engine_type'] ]['name'];

				if ( $model_info['need_pk'] ) {
					$sql = <<<sql
                CREATE TABLE IF NOT EXISTS `{$this->tableName}` (
                `id`  int(10) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键' ,
                `{$field['name']}`  {$field['field_define']} {$default} COMMENT '{$field['remark']}' ,
                PRIMARY KEY (`id`)
                )
...
sql;
		...
		protected function updateField( $field ) {
			...
			//获取默认值
			$default = $field['field_default'] != '' ? ' DEFAULT ' . $field['field_default'] : '';

			$sql = <<<sql
            ALTER TABLE `{$this->tableName}`
CHANGE COLUMN `{$last_field}` `{$field['name']}`  {$field['field_define']} {$default} COMMENT '{$field['remark']}' ;
sql;
			$res = M()->execute( $sql );
			return $res !== false;
		}
		...
		public function deleteField( $field ) {
			...
			$sql = <<<sql
            ALTER TABLE `{$this->tableName}` DROP COLUMN `{$field['name']}`;
sql;

			$model = M('model')->where(array('id'=>$field['model_id']))->find();

			$_fields = M( $model['name'] )->getFields();
			if ( in_array( $field['name'] , $_fields ) ) {
				$res = M()->execute( $sql );
				return $res !== false;
			}else{
				return true;
